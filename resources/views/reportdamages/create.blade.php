...
<div class="container mx-auto px-4 py-8">
    <div class="max-w-3xl mx-auto">
        <h1 class="text-2xl font-bold text-gray-800 mb-6">Create New Report Damage</h1>

        <form action="{{ route('reportdamages.store') }}" method="POST">
            @csrf
                ...
                <div class="grid grid-cols-1 gap-6">
                    ...
                    <div>
                        ...
                        <textarea name="deskripsi_kerusakan" id="deskripsi_kerusakan" 
                                  ...
                                  rows="4" required>{{ old('deskripsi_kerusakan') }}</textarea>
                    ...
                    <div>
                        ...
                        <input type="datetime-local" name="tanggal_lapor" id="tanggal_lapor"
                               class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500"
                               value="{{ old('tanggal_lapor', now()->format('Y-m-d\TH:i')) }}" required>
                        @error('tanggal_lapor')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
